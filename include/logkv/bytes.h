...
private:
  ...
  static constexpr std::array<int, 256> hex_lookup =
    logkv_detail::createHexLookup();
...
public:
  ...
  static Bytes decodeHex(const char* hexData, size_t hexSize) {
    ...
    size_t bytesSize = hexSize / 2;
    Bytes result(bytesSize);
    for (size_t i = 0; i < bytesSize; ++i) {
      const unsigned char hchar = hexData[2 * i];
      const unsigned char lchar = hexData[2 * i + 1];
      const int hval = hex_lookup[hchar];
      const int lval = hex_lookup[lchar];
      if (hval == -1 || lval == -1) {
        throw std::invalid_argument("Hex string has invalid characters");
      }
      result.data_[i] = static_cast<char>((hval << 4) | lval);
    }
    return result;
  }
  ...
  static Bytes decodeHex(const Bytes& hexBytes) {
    return decodeHex(hexBytes.data_, hexBytes.size_);
  }
  ...
  static Bytes encodeHex(const char* data, size_t size, bool upper = false) {
    ...
    if (upper) {
      for (size_t i = 0; i < size; ++i) {
        ...
        result.data_[i * 2] = logkv_detail::hexEncodeLookupUpper[byte >> 4];
        result.data_[i * 2 + 1] =
          logkv_detail::hexEncodeLookupUpper[byte & 0x0F];
      }
    } else {
      for (size_t i = 0; i < size; ++i) {
        const uint8_t byte = static_cast<uint8_t>(data[i]);
        result.data_[i * 2] = logkv_detail::hexEncodeLookupLower[byte >> 4];
        result.data_[i * 2 + 1] =
          logkv_detail::hexEncodeLookupLower[byte & 0x0F];
      }
    }
    return result;
  }
  ...
  static Bytes encodeHex(const Bytes& bytes, bool upper = false) {
    return encodeHex(bytes.data_, bytes.size_, upper);
  }
